...
class OrchestratorVoiceTester:
    ...
    async def run(self):
        ...
        print("  2. Send to Gemini 2.0 Live API for transcription (STT)")
        print("  3. Transcribed text is sent to Strands Agent (Claude)")
        print("  4. Agent streams text response -> TTS -> Audio Output (Immediate Feedback)")
        print("  5. Agent executes Cloud Code tools if needed (e.g., 'read this file')")
        print("  6. Tool status updates are streamed -> TTS -> Audio Output")
        print("  7. Agent uses tool results to generate final response")
